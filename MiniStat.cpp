 ...
 *
 ...
 1/19/2019:5:50pm -- removed the MCP49XX since we'll be using a smaller DAC
 in the future
...
//values are still smaller than Gamry. I'm thinking I may need to sample a bit
//before the end of the pulse and pick the highest current or something
//
//
...
void MiniStat::runNPV(uint8_t user_gain, uint32_t pulse_period,
					  uint32_t pulse_width, int16_t startV, int16_t endV,
					  uint32_t quietTime, uint8_t range)
{
	pStat.disableFET();
	pStat.setGain(user_gain);
	pStat.setRLoad(0);
...
void MiniStat::runAmp(uint8_t user_gain, int16_t pre_stepV, uint32_t quietTime,
